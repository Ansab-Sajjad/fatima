...
import { Icon_User_data } from "../../fixtures/Icon Exchange Credentials/Login_Credentials.cy";
...
function Icon_user_Login() {
	...
	it('3- Enter Wrong password.', () => {
	    cy.get(':nth-child(2) > :nth-child(1) > .form-controls > input').clear().type(Icon_User_data.wrong_Password , {delay: 100})
	    cy.get('.ico').click()
	    expect(true).to.equal(true)
	...
	it('6- Enter wrong Email.', () => {
	    cy.wait(2000);
	    cy.get(':nth-child(1) > .form-group > .form-controls > input').clear().type(Icon_User_data.wrong_Email, {delay: 100})
	    expect(true).to.equal(true)
	...
	it("10- Enter Correct Email.", () => {
		cy.wait(2000);
		cy
			.get(":nth-child(1) > .form-group > .form-controls > input")
			.clear()
			.type(Icon_User_data.Correct_Email, { delay: 100 });
		expect(true).to.equal(true);
	});

	it("11- Enter Correct Password.", () => {
		cy
			.get(":nth-child(2) > :nth-child(1) > .form-controls > input")
			.clear()
			.type(Icon_User_data.Correct_Password, { delay: 100 });
		expect(true).to.equal(true);
	});
	...
	it.skip("13- Verifying the Toast message", () => {
		cy.wait(1000);
		cy
			.get(".Toastify__toast-body > :nth-child(2)", { timeout: 10000 })
			.should("be.visible")
...
export default Icon_user_Login;
